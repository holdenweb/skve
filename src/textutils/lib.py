...
class YesNoText(Text):
    ...
    def render(self):
        return Text.from_markup(
            f"{self.msg}[@click='{self.yes_action}']yes[/] [@click='{self.no_action}']no[/]",
        )
        ...
        with Center(classes=f'id-{self.id} save-cancel-button-row'):
            with Horizontal(classes=f'id-{self.id} save-cancel-buttons'):
                yield Button(
                    'Save',
                    variant='primary',
                    classes=f'save id-{self.id} save-cancel-save-button',
                )
                yield Button(
                    'Cancel',
                    variant='error',
                    classes=f'cancel id-{self.id} save-cancel-cancel-button',
                )
