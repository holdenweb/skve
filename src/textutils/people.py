...
class MainScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Horizontal(VerticalScroll(id="buttons"), self.content_pane, id="body")
...
class PersonButton(Button):
    ...
    def on_button_pressed(self):
        ...
        self.content_pane.add_result_row("name", self.person['name'], clickable=True)
        for k in self.person.keys():
            if k not in {'name', 'id'}:
                self.content_pane.add_result_row(
                    k, self.person[k], widget_type=fields[k].widget_type
                )
...
class ResultRow(Widget):
    ...
    def on_click(self, e):
        if self.clickable:
            app.push_screen(
                KeyValueEditScreen(self.key, self.value, validators=self.validators, widget_type=self.widget_type),
                self.stash_result,
            )
